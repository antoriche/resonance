...
async function compareEmbeddings() {
  ...
  console.log(
    `Last embedding (first 10 values): ${JSON.stringify(last.embedding.slice(0, 10))}`,
  );
  console.log(
    `Last - 2 embedding (first 10 values): ${JSON.stringify(lastMinus2.embedding.slice(0, 10))}`,
  );

  // Compare if they're identical
  const areIdentical =
    JSON.stringify(last.embedding) === JSON.stringify(lastMinus2.embedding);
  console.log(`Are they identical? ${areIdentical ? "YES" : "NO"}`);

  if (!areIdentical) {
    // Calculate cosine similarity
    const dotProduct = last.embedding.reduce(
      (sum: number, val: number, idx: number) =>
        sum + val * lastMinus2.embedding[idx],
      0,
    );

    const magnitudeA = Math.sqrt(
      last.embedding.reduce((sum: number, val: number) => sum + val * val, 0),
    );
    const magnitudeB = Math.sqrt(
      lastMinus2.embedding.reduce(
